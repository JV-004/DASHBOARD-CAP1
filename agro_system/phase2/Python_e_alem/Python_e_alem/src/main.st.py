...
from database.db_handler import conexao_db
...
def gerando_relatorio():
    base_dir = os.path.dirname(__file__)
    caminho = os.path.join(base_dir, 'data', 'relatorio_pragas.json')
...
def main():
    ...
    while True:
        try:
            selecione_cultura = st.selectbox(
                "Selecione uma cultura",
                (constants.LISTAS_CULTURAS))

            st.write('Relatorio')
            st.dataframe(gerando_relatorio())

            #download relatorio completo em formato txt
            #download relatorio resumido em formato txt
            ...
            match opcao:
                case 1:
                    ...
                    if cultura.capitalize() not in constants.LISTAS_CULTURAS:
                        print(
                            '\033[1;31mCultura inválida. Tente novamente.\033[m')
                        continue
                    ...
                    print(
                        f"{'Nível de Risco:':<20} {risco.upper() if risco else 'Desconhecido'}")
                ...
                case _:
                    print(
                        '\033[1;31mOpção inválida. Escolha 1, 2, 3 ou 4.\033[m')
